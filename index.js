...
function carregarConfigGrupo(grupoId) {
  ...
  if (!fs.existsSync(file)) {
    const configPadrao = {
      ...
      blockgp: false,
      bemvindo: false,
      legendabv: "Olá #membro# seja bem vindo ao #grupo#"
    };
  ...
  try {
    ...
    data.blockgp ??= false;
    data.bemvindo ??= false;
    data.legendabv ??= "Olá #membro# seja bem vindo ao #grupo#";
    return data;
  } catch {
    return {
      listanegra: [],
...
async function executarAgendamento(grupoId, comandoFallback) {
  try {
    if (!sockGlobal) throw new Error('Sock não inicializado');   
    ...
    comandosPendentes.delete(grupoId);
    console.log(`✅ Executado: ${comandoFallback} no grupo ${grupoId}`);
  } catch (err) {
    if (err.message.includes('Connection Closed')) {
      comandosPendentes.set(grupoId, comandoFallback);
      console.log(`⚠️ Sock fechado. Comando ${comandoFallback} pendente para ${grupoId}`);
    } else {
      console.log(`❌ Falha ao executar ${comandoFallback} em ${grupoId}: ${err.message}`);
    }
  }
}
// Antilink
...
function contarMensagem(from, sender) {
  console.log(`[Contador] mensagem de ${sender} em ${from}`);
...
if (isGroup && msgLower === 'menu') {
  ...
  try {
    ...
    const thumbnail = await getBuffer('https://i.postimg.cc/d1zzMMLf/20250702-122138.jpg');
...
┏│ ✰✰✰✰✰✰
┃│Oiê ${userTag}
┃│ᴛᴇᴍᴘᴏ ᴏɴ: ${uptimeHoras}h ${uptimeMin}m ${uptimeSeg}s
┃│ʜᴏʀᴀ: ${hora}
┃│ᴅᴏɴᴏ: ${isDono ? '✅' : '❌'}
┃│ᴀᴅᴍ: ${admStatus}
┃│ʙᴏᴛ: ${info.nomebot}
...
if (isGroup && msgLower === 'menuadm') {
  ...
  const thumbnail = await getBuffer('https://files.catbox.moe/b10rwp.jpg');
...
if (isGroup && /^([!.])?(ban|b)\b/i.test(msgLower)) {
  ...
  if (alvos.size === 0) {
    await sock.sendMessage(from, { text: '❌ Marque alguém com @ ou responda à mensagem da pessoa que deseja banir.' }, { quoted: msg });
    return;
  }
  const isLink = (text) => {
  ...
  for (const jid of alvos) {
    ...
    if (quotedMessage) {
      ...
      if (!isLink(conteudoAlvo) && !isTagAll(conteudoAlvo)) {
        await sock.sendMessage(from, {
          text: '❌ Só posso banir se a mensagem respondida for suspeita (link ou marcação a todos). Marcar uma pessoa não é suficiente.',
          quoted: msg
        });
        continue;
      }
    ...
    try {
      ...
      try {
        const stickerBuffer = fs.readFileSync('./dados/figurinhas/ban.webp');
        await sock.sendMessage(from, { sticker: stickerBuffer }, { quoted: msg });
      } catch (e) {
        console.error('Erro ao enviar figurinha ban:', e);
      }
      await sock.sendMessage(from, {
...
if (msgLower === 'fs') {
  ...
  const target = quotedParticipant ? quotedParticipant : (mentionedJid.length > 0 ? mentionedJid[0] : null);
  const texto = target
    ? `🔪⚔️ @${sender.split('@')[0]} eliminou friamente @${target.split('@')[0]}! 💀☠️`
...
// DOWNLOADS ///
  ...
  const legenda = `*_🌸𝑩𝑨𝑰𝑿𝑨𝑵𝑫𝑶🎶𝑨𝑼𝑫𝑰𝑶💕..._*\n`;
  ...
  exec(`yt-dlp --no-playlist --no-warnings --no-call-home --no-check-certificate --ignore-errors --no-overwrites -x --audio-format mp3 --cookies "/storage/D72F-0D01/Download/SanizinhaBot/dados/cookies/youtube.com_cookies.txt" -o "${output}" "${videoUrl}"`, async (err, stdout, stderr) => {
    if (err || stderr.includes('Sign in to confirm your age')) {
      console.error('Erro ao baixar:', err || stderr);
      await sock.sendMessage(from, {
        text: '❌ Este vídeo possui restrição de idade e não pode ser baixado.\nTente outro link ou nome de música.',
        quoted: msg
      });
      return;
    }
    ...
    if (fs.existsSync(output)) {
      ...
      await sock.sendMessage(from, { text: '❌ Erro: arquivo de áudio não encontrado.' }, { quoted: msg });
    }
  });
}
...
try {
 ...
 // Iniciar jogo da velha
  ...
  if (msgLower.includes('bot')) {
    ...
    await sock.sendMessage(from, { text: `🎮 *Jogo da Velha iniciado contra o Bot!*\n${jogoVelha.jogar(from, sender, 0)}` }, { quoted: msg });
  } else {
    const mencionado = msg.message.extendedTextMessage?.contextInfo?.mentionedJid?.[0];
    if (!mencionado) {
      await sock.sendMessage(from, { text: '👥 Marque alguém para jogar contra ou use "jv bot" para jogar contra o bot.' }, { quoted: msg });
      return;
    }
    ...
    await sock.sendMessage(from, { text: `🎮 *Jogo da Velha iniciado!*\n${jogoVelha.jogar(from, sender, 0)}` }, { quoted: msg });
  }
}
...
if (
  ...
  await sock.sendMessage(from, {
    text: `🕐 ${tipo === 'gpa' ? '*ABRIR*' : '*FECHAR*'} grupo agendado para *${agendamentos[from][tipo]}*.`
  }, { quoted: msg });
}
// Comando .antifake
...
if (update.action === 'add' && configGrupo.bemvindo) {
  if (!groupMetadata) {
    try {
      ...
      console.error('Erro obter metadata grupo:', e);
    }
  }
...
async function reagir(jid, emoji, msgRef) {
  try {
...
if (update.action === 'add') {
  const groupId = update.id;
  const userId = update.participants[0];
  const configPath = `./dados/grupos/${groupId}.json`;
  if (fs.existsSync(configPath)) {
    const configGrupo = JSON.parse(fs.readFileSync(configPath));
    if (configGrupo.listanegra?.includes(userId)) {
      await sock.groupParticipantsUpdate(groupId, [userId], 'remove');
      await sock.sendMessage(groupId, { text: ` oi, tchau kkk:\n${userId.replace(/@s\.whatsapp\.net/, '')}` });
    }
  }
}
      // 🔒 AntiFake
